...
open class UICheckboxClassic: UIView {
    ...
    open var id = NSUUID().uuidString // random ID for each checkbox button created
...
//MARK: configuration functions
    fileprivate func checkboxConfig() {
        ...
        borderWidth = button.frame.height / 8
    ...
    fileprivate func configCheckmarkConstraints() {
        ...
        NSLayoutConstraint(item: self.checkmark!, attribute: .centerY, relatedBy: .equal, toItem: self.button, attribute: .centerY, multiplier: 0.7,   constant: 0).isActive = true
        NSLayoutConstraint(item: self.checkmark!, attribute: .centerX, relatedBy: .equal, toItem: self.button, attribute: .centerX, multiplier: 1.4,   constant: 0).isActive = true
    }
    ...
    fileprivate func deselectAnimate() {
        UIView.animate(withDuration: 0.1, delay: 0, usingSpringWithDamping: 0.2, initialSpringVelocity: 3, options: .curveEaseIn, animations: {
            self.layoutIfNeeded()
        }, completion: {
            finished in
            self.checkmark.isHidden = true
